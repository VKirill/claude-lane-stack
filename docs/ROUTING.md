...
|-------|---------|---------|
| `plan_critique` | on · `advisory` · **`jev`** | Structural extracts owns/verify/DAG. **Jev** (`typesafe/jev-1.13` via OpenRouter Decisions) sets `ship/revise/revise_required`, fat-task split, risk on `run.yaml`, and can demote soft warns. Leftover yaml `provider: agy` is remapped to Jev at load (no `adoc` needed). `structural` skips the model. Qwen/Codex/… still work if set explicitly. Night-fix compile, leftover verify class, `run-init --brief`, `jev-intent`, and `browser-qa` REPORT also call Jev as a typed judge — never as a writer. |
...
| `browser_qa` | on · **`jev`** · `typesafe/jev-1.13` · `chrome-qa` · `auto` | Live browser QA: DOM table → Jev click/type (`browser-qa-jev`). `codex` / `claude` still available |

`browser_qa` provider is `jev` (`browser-qa-jev`: CDP snapshot → Jev Choice → click;
TYPE_TEXT via a small OpenRouter model; DONE checked against Expected), `codex`
(`codex exec` + browser plugins), or `claude` (Haiku + `chrome-devtools` MCP). `backend`
