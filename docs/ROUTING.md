...
|-------|---------|---------|
| `plan_critique` | on · `advisory` · **`jev`** | Structural extracts owns/verify/DAG. **Jev** (`typesafe/jev-1.13` via OpenRouter Decisions) sets `ship/revise/revise_required`, fat-task split, risk on `run.yaml`, and can demote soft warns. Other providers (AGY/Qwen/…) still work. `structural` skips the model. Night-fix compile, leftover verify class, `run-init --brief`, `jev-intent`, and `browser-qa` REPORT also call Jev as a typed judge — never as a writer. |
