...
flowchart LR
    ...
    PM --> W1["⚡ AGY<br/>быстрая запись"]
    PM --> W2["🔧 Grok<br/>тяжёлая запись"]
    PM --> W3["🔍 Codex<br/>ревью"]
    W1 --> PM
    W2 --> PM
    W3 --> PM
...
Внутри одного run задачи AGY/Grok теперь продолжают тёплые conversation через
`lane-session`. Несколько задач в run принимаются **по мере готовности**
(`lane-poll` + `MODE=start`/`finish`) — PM не ждёт самую долгую, чтобы принять
уже готовые. Связанные последовательные задачи сохраняют контекст, а реально
параллельные получают разные слоты. Стандартный срок жизни — семь успешных задач.
