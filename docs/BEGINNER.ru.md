...
> [!IMPORTANT]
> Если пишущий lane **умирает примерно через 2 минуты** — чаще всего это **Claude убивает foreground Bash**, а не баг вашего кода. Текущий стек запускает provider через `lane-ctl`: detached-процесс и lifecycle events. Подробнее: [LANE-EXEC.md](LANE-EXEC.md).
