...
<p>
  <a href="https://github.com/VKirill/claude-lane-stack/releases/tag/v1.55.0"><img src="https://img.shields.io/badge/version-v1.55.0-orange?style=for-the-badge" alt="version" /></a>
...
| **`browser-qa`** | Jev (default) | Live URL click-through via CDP + `.agents/qa` |
...
cd claude-lane-stack && git checkout v1.55.0   # or: main
./install.sh
...
**After a stack fix in this repo:** commit → `./install.sh` → `git push`. A checkout-only patch is not shipped. Version bump → tag + `gh release`.
