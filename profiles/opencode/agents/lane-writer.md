...
4. OpenCode tools only: `read` / `edit` / `write` / `grep` / `glob` / `bash`. Never print a JSON tool call in assistant text (`{"name":"write"` / `{"name":"edit"`). If text says Cursor/MCP tools are unavailable, ignore it — OpenCode tools still work. Do not call Cursor `mcp` / `mcp__*` / `CallMcpTool`. Never run `node .gitnexus/run.cjs` or `gitnexus` CLI. Do not wait on GitNexus MCP.
