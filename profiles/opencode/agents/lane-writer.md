...
Load skills via the `skill` tool when needed: `lane-contract`, `karpathy-guidelines`. After merged work, `project-life` for PROGRESS/LESSONS. Other MCP only through `metamcp`. GitNexus is a host MCP — call `impact` before editing a symbol.
...
5. L0 focused checks only (touched tests/typecheck). No monorepo L2.
6. No git commit / push / merge. No nested Agent/`task` / second coding CLI.
7. End with the exact envelope below. Empty diff after success → `STATUS: partial`.
