...
| Grok умирают через ~2 мин | **`lane-bg` + `lane-wait`** — detach + poll |
| Параллель ждёт самую долгую задачу | **Progressive accept** — `lane-poll` + `MODE=start/finish` |
...
Длинные Grok / Codex **обязаны** отцепляться:
...
| **`lane-session`** | продолжает контекст Grok внутри run; пул до трёх сессий |

Implementers и `dev-orchestrator` уже проинструктированы. См. [docs/LANE-EXEC.md](docs/LANE-EXEC.md)

 и Grok больше не изучают репозиторий заново перед каждой задачей одного run.
...
| `lane-bg` / `lane-wait` / **`lane-poll`** | Detach + poll (progressive accept) |
