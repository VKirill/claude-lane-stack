...
### Маленький ИИ-завод для одного человека · **v1.3.1**

**Мульти-агентная оркестрация для Claude Code** — вы говорите с одним ИИ-менеджером проекта,
он раздаёт задачи воркерам (Grok / Codex), проверяет результат
...
| Grok умирают через ~2 мин | **`lane-ctl` + user-systemd** — процесс переживает cleanup хоста |
| Параллель ждёт самую долгую задачу | **Event-driven accept** — detached Grok + события + отдельный verify-пул |
...
| ⚡🔧 Пишущие |, Grok *(опц.)* | Делают карточки (через `lane-bg`) |
...
cd claude-lane-stack && git checkout v1.3.1 # или main
./install.sh
...
📖 Гайд: **[docs/BEGINNER.ru.md](docs/BEGINNER.ru.md)** · Релиз: **[v1.3.1](https://github.com/VKirill/claude-lane-stack/releases/tag/v1.3.1)**
...
Длинные Grok запускаются через типизированный control plane:
...
| Инструмент | Роль |
|------------|------|
| **`lane-ctl`** | start/status/events/tail/retry/cancel/verify |
...
Read-only `lane-supervisor` и `dev-orchestrator` уже проинструктированы; verify-пул отдельный, 2 по умолчанию, максимум 10. См. [docs/LANE-EXEC.md](docs/LANE-EXEC.md)
...
| **`lane-ctl`** | Типизированный lifecycle control + отдельная verification |
...
Чаще **нет**. Claude убивает **foreground Bash** и может убрать обычный nohup-потомок. Текущий стек стартует через **`lane-ctl`**, а `lane-bg` использует transient user-systemd service. См. [docs/LANE-EXEC.md](docs/LANE-EXEC.md). После апдейта — **новая сессия** PM.
...
| 🚀 Релиз v1.3.1 | [GitHub Releases](https://github.com/VKirill/claude-lane-stack/releases/tag/v1.3.1) |
