...
""")
    ...
    def test_opencode_writer_contract_forbids_cli_impact_and_json_dumps(self) -> None:
        ...
        self.assertIn("once, whole", agent)
        self.assertIn("Do not pick silently", agent)
        self.assertIn("Use bounded ranges for large files", prompt)
        self.assertIn("report meaningful assumptions", prompt)
        self.assertIn("Removing obsolete owned code is allowed", prompt)
        self.assertNotIn("PROMPT_SHA256:", prompt)
        emergency = (ROOT / "agents/codex/instructions/writer-emergency.md").read_text()
        self.assertIn("do not run tests", emergency)
        self.assertIn("canonical `LANE_REPORT`", emergency)
        self.assertNotIn("Write `ARTIFACT_DIR/report.md`", emergency)
        coordinator = (ROOT / "plugins/lane-stack/agents/emergency-writer.md").read_text()
        self.assertIn('lane-ctl verify --run-dir "$RUN_DIR" --task-file "$TASK_FILE"', coordinator)
        self.assertIn("Failed checks or missing receipts", coordinator)
